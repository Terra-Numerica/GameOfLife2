...
// ---------------------- Navbar functions ----------------------
    ...
    addButtonToNavbar({textButton, onclickFunction, hover = true, classesOfTheButton = ["a"]} = {}){
        // If no text is provided, don't add a default button. This avoids
        // accidental creation of 'click me' buttons when the caller forgot to
        // pass parameters. Caller should explicitly pass `textButton` and
        // optionally `onclickFunction`.
        if (!textButton) {
            return null;
        }
        ...
        // Use a no-op click handler if none provided, instead of alerting.
        const onClick = onclickFunction || (() => {});

        Banner.create_button({ text: textButton, onClick: onClick, classes: classesOfTheButton });
